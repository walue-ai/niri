...
pub fn setup_multi_screen_display(
    ...
    for (i, (output_name, texture_handle)) in capture_state.output_textures.iter().enumerate() {
        ...
        commands.spawn((
            meshes.add(Plane3d::new(Vec3::Z, Vec2::splat(1.0)).mesh()),
        ...
        commands.spawn((
            Text::from_section(
                format!("Niri Output: {}", output_name),
                TextStyle {
                    font_size: 16.0,
                    color: Color::WHITE,
                    ..default()
...
pub fn update_screen_transforms(
    ...
    let rotation = Quat::from_rotation_y(time.elapsed_seconds() * rotation_speed);
...
pub fn handle_screen_interaction(
    ...
    if input.just_pressed(KeyCode::Space) {
        for (mut transform, screen_display) in query.iter_mut() {
            transform.scale *= 1.1;
            info!("Scaled up display for output: {}", screen_display.output_name);
        }
    }
    ...
    if input.just_pressed(KeyCode::KeyR) {
        for (mut transform, screen_display) in query.iter_mut() {
            ...
            info!("Reset transform for output: {}", screen_display.output_name);
        }
    }
}
